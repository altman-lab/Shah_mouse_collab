...
vars_of_interest <- c("group","visit")
...
write_tsv(key, path="data_clean/MGI_ensembl.key.txt")
```

```{r key, message=FALSE}
key.pc <- read_tsv("data_clean/MGI_ensembl.key.txt") %>% 
...
```{r}
#Counts table
write_csv(as.data.frame(dat.pc.abund.norm.voom$E),
          "data_clean/Shah.clean.counts.csv")
```
